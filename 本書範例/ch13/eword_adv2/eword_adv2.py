...
def search_data():
    while True:
        ...
        key_id = CkeckKey(eword)
        if key_id != "":      # 判斷鍵是否存在
            print("中文翻譯：{}".format(datas[key_id][key_id]))
        else:
            print("{} 未建立!\n".format(eword))  
        input("請按任意鍵繼續查詢…")    
...
def edit_data():
    while True:
        ...
        key_id = CkeckKey(eword)
        if key_id != "":      # 判斷鍵是否存在
            print("原來中文翻譯：{}".format(datas[key_id][key_id]))  
            cword=input("請輸入中文翻譯：")
            word={eword:cword}
            datas[key_id]=word 
...
def delete_data():
    while True:
        ...
        key_id = CkeckKey(eword)   
        if key_id != "":      # 判斷鍵是否存在
            print("確定刪除 {} 的資料!：".format(datas[key_id][key_id]))
            yn=input("(Y/N)?")
            if (yn=="Y" or yn=="y"):
...
url = 'https://chiouapp01-dedce.firebaseio.com/English_adv2'
